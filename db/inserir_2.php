...
<?php
    ...
    if(count($_POST) > 0){
        ...
        if(!filter_var($dados['filhos'],FILTER_VALIDATE_INT,$filhosConfiguracao) && $dados['filhos'] !=0){
            $erros['filhos'] = "Quantidade de filhos é inválida(0-20)";
        }

        $salarioConfiguracao = ["options" => ["decimal" => ",","min_range" => 1045 ]];
        if(!filter_var($dados['salario'], FILTER_VALIDATE_FLOAT, $salarioConfiguracao)){
    ...
    <div class="form-row">
        <div class="form-group col-md-8">
            ...
            <input type="text" class="form-control <?= $erros['nome'] ? 'is-invalid' : ''?>" id="nome" name="nome"
                placeholder="Informe o seu nome" value="<?= $dados['nome'] ?>">
        ...
        <div class="form-group col-md-4">
            ...
            <input type="text" class="form-control <?= $erros['nascimento'] ? 'is-invalid' : ''?>" id="nascimento"
                name="nascimento" placeholder="Ex.: 01/01/2000" value="<?= $dados['nascimento'] ?>">
            <div class="invalid-feedback">
                <?= $erros['nascimento'] ?>
            </div>
        </div>
    ...
    <div class="form-row">
        <div class="form-group col-md-6">
            ...
            <input type="text" class="form-control <?= $erros['email'] ? 'is-invalid' : ''?>" id="email" name="email"
                placeholder="Informe o seu e-mail" value="<?= $dados['email'] ?>">
            <div class="invalid-feedback">
                <?= $erros['email'] ?>
            </div>
        </div>
        <div class="form-group col-md-6">
            ...
            <input type="text" class="form-control <?= $erros['site'] ? 'is-invalid' : ''?>" id="site" name="site"
                placeholder="Ex.: https://www.site.com" value="<?= $dados['site'] ?>">
            <div class="invalid-feedback">
                <?= $erros['site'] ?>
            </div>
        </div>
    ...
    <div class="form-row">
        <div class="form-group col-md-6">
            ...
            <input type="number" class="form-control <?= $erros['filhos'] ? 'is-invalid' : ''?>" id="filhos"
                name="filhos" placeholder="Nº de Filhos" value="<?= $dados['filhos'] ?>">
            <div class="invalid-feedback">
                <?= $erros['filhos'] ?>
            </div>
        </div>
